---
title: "Chapter 5"
subtitle: "Summarizing Bivariate Data"
author: "Behavior Variables"
institute: ""
date: "`r Sys.Date()`"
output:
...
---
background-image: url(5_present_files/figure-html/corr1.png)
background-size: 1000px
background-position: 15% 100%

### The Correlation Coefficient r
The correlation coefficient $r$ is a measure of the strength of the **LINEAR** realtionship between two variables. 
---
...
.output[
```{r echo=FALSE}
phs_value("CHRONIC_TRUANTS_RATE_SCHOOL_per")
```
]
]
---
...
.output[
```{r echo=FALSE}
phs_value("STUDENT_MOBILITY_RATE_SCHOOL_perALL")
```
]
]
---
...
.output[
```{r echo=FALSE}
phs_value("ATTENDANCE_RATE_SCHOOL_perALL")
```
]
]
---
...
--
Dropout Rate for Female vs Male
--
.pull-left[
```{r echo=FALSE, warning=FALSE, message=FALSE}
dropout <- sixco %>% ggplot(mapping = aes(x = DROPOUT_RATE_SCHOOL_perMALE, y = DROPOUT_RATE_SCHOOL_perFEMALE)) +
  geom_point(color="Blue") +
  labs(x = "Male Dropout Rate (Percent)", 
       y = "Female Dropout Rate (Percent)", 
       title = "Dropout Rates", 
       subtitle = "Six County High Schools")
dropout
```
...
.output[
```{r echo=FALSE, warning=FALSE, message=FALSE}
cor(sixco$DROPOUT_RATE_SCHOOL_perFEMALE,sixco$DROPOUT_RATE_SCHOOL_perMALE, use = "pairwise.complete.obs")
```
]
Female Dropout Rate for PHS
.output[
```{r echo=FALSE}
phs_value("DROPOUT_RATE_SCHOOL_perFEMALE")

```
]

Male Dropout Rate for PHS
.output[
```{r echo=FALSE}
...
---
### The 5 Properties of the Correlation Coefficient

  1. Change of units has no effect on $r$. 

--

  2. Correlation does not require explanatory /response. 

--

  3. Correlation is always between $-1$ and positive $1$.

--

  4. Perfect linear relationship produces $r=1$ or $r=-1$. 

--

  5. $r$ is a measure of the **LINEAR** relationship. 

---
background-image: url(5_present_files/figure-html/calc_data.png)
...
### Scratch Work
.pull-left[
```{r echo=FALSE, message=FALSE, warning=FALSE, out.width = '700px'}
sixco %>% ggplot(mapping = aes(x= ATTENDANCE_RATE_SCHOOL_perALL, y = ACT_COMP_SCHOOL)) +
  geom_point() + 
  theme(aspect.ratio=.5625)
```
]
.pull-right[Correlation]
