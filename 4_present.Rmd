---
title: "Chapter 4"
subtitle: "Numerical Methods for Describing Data"
author: "AP STATISTICS"
institute: "PHS"
...
background-image: url(4_present_files/figure-html/000005.png)
...
---
background-image: url(4_present_files/figure-html/money.png)

---
## Summary: You should be able to:
 1. Calculate the mean, median and proportion by hand.
 
 • Understand (read and write) ∑ notation.
 
 2. Find the sample mean, median and proportion on your calculator or software output.
 
 3. Understand the relationship between the sample mean $\bar{x}$ and the population mean $\mu$.
 
  • vocabulary: *unbiased estimator*

 4. Understand the relationship between the sample proportion (p) and the population proportion $\pi$.
 
## And:
 5. Compare the mean and median for skewed and symmetric distributions. 
 
 6. Find a trimmed mean.

---
### 1. Calculate the mean, median and proportion by hand.
Find the mean, median and proportion greater than each for the following data:
...
.pull-left[
```{r echo = FALSE}
spend_per_student_kane <- rc17 %>% 
  filter(COUNTY == "Kane") %>% 
  group_by(DISTRICT_NAME) %>% 
  summarise(Spend = mean(INSTRUCT_EXPEND_PER_PUPIL_DISTRICT201516))

stripchart(spend_per_student_kane$Spend, 
           method = "jitter", 
           pch = 19, 
           main = "Expenditures per Student (Kane County)", 
           xlab = "Dollars", 
           ylab = "District",
           col = "Blue")
```
...
.pull-right[
```{r echo=FALSE}
spend_per_student_table_kane <- rc17 %>% 
  filter(COUNTY == "Kane", SCHOOL_TYPE_NAME == "HIGH SCHOOL") %>% 
  group_by(DISTRICT_NAME) %>% 
  summarise(Spend = mean(INSTRUCT_EXPEND_PER_PUPIL_DISTRICT201516), Schools = n())
  
spend_per_student_table_kane
```
]
---
### 2. Find the sample mean, median and proportion on your calculator or software output.
#### Expenditures per Student for Kane County
```{r echo=FALSE}
summary(spend_per_student_table_kane)
```
...
---
background-image: url(4_present_files/figure-html/ub_est.png)
background-size: 550px
background-position: 15% 65%

### 3. Understand the relationship between the sample mean $\bar{x}$ and the population mean $\mu$.
 
Since the center of the $\bar{x}$ distribution ~~is~~ *appears to be* the same as the population mean $\mu$, we can say that $\bar{x}$ is an **unbiased estimator** of µ.
...
spend_stu_plot <- sixco %>% 
  ...
  labs(x = "Spend", y = "", title = "Expenditures per Student by District (2016) ", subtitle = "Six County Region", caption = "Only high school districts were included.")
...
spend_stu_plot <- rc17 %>% 
  ...
  geom_histogram(binwidth = 500, color = "Blue", fill = "LightBlue") +
  scale_x_continuous(labels = dollar) +
  labs(x = "Spend", y = "", title = "Expenditures per Student (2016) ", subtitle = "Six County Region", caption = "Only high school districts were included.")
```
...
spend_stu_plot <- ggplot(mapping = aes(x = spend_stu_tbl$spend_stu)) +
  geom_histogram(binwidth = 500, color = "Blue", fill = "LightBlue") +
  scale_x_continuous(labels = dollar) +
  labs(x = "Spend", y = "", title = "Expenditures per Student by District (2016) ", subtitle = "All Counties", caption = "Only high school districts were included.")
...
---
```{r echo = FALSE, warning=FALSE}
spend_stu_plot <- spend_stu_plot +
   geom_segment(aes(x=5912, y=80, xend=5912, yend=0), arrow = arrow(length = unit(.03, "npc")), color="DarkGreen") +
  geom_label(aes(x=5912, y=80,  label = "Median \n $5,912"), nudge_x = -400)
...
```{r echo = FALSE, warning=FALSE}
spend_stu_plot <- spend_stu_plot + 
  geom_segment(aes(x=6571, y=80, xend=6571, yend=0), arrow = arrow(length = unit(.03, "npc")), color="DarkRed") + 
  geom_label(aes(x=6571, y=80,  label = "Mean \n $6,571"), nudge_x = 400) 
...
```{r echo = FALSE, warning=FALSE}
spend_stu_plot <- spend_stu_plot +
  geom_segment(aes(x=12405, y=80, xend=12405, yend=0), arrow = arrow(length = unit(.03, "npc")), color="DarkBlue") + 
  geom_label(aes(x=12405, y=80,  label = "PHS \n $12,405"), nudge_x = 400)
...
---
```{r include=FALSE}
spend_per_student_table <- rc17 %>% 
  filter(COUNTY %in% c("Cook", "Lake", "Will", "Kane", "Kendall", "Dupage"), DISTRICT_NAME  != "City of Chicago SD 299", DISTRICT_TYPE_NAME == "HIGH SCHOOL") %>% 
  group_by(COUNTY, DISTRICT_NAME) %>%
  summarise(spend = mean(INSTRUCT_EXPEND_PER_PUPIL_DISTRICT201516, na.rm = TRUE)) %>% 
  arrange(desc(spend)) %>% 
  select(District =  DISTRICT_NAME, County  = COUNTY, Spend = spend)
```
Per Pupil Spending in High School Districts
```{r echo=FALSE}
datatable(spend_per_student_table) %>% formatCurrency("Spend")
```
